...
class KnowledgeBaseResearcher:
    ...
    def __init__(self, knowledge_base_file: Path, config: Dict[str, Any] = None, cache_manager=None):
        """
    ...
    async def _load_pages_data(self) -> bool:
        ...
        try:
            if not self.knowledge_base_file.exists():
                self.logger.error(f"Knowledge base file not found: {self.knowledge_base_file}")
                return False
            
            # Check if file has been modified since last load
            file_mtime = datetime.fromtimestamp(self.knowledge_base_file.stat().st_mtime)
            if self._last_loaded and file_mtime <= self._last_loaded:
                return True  # No need to reload
            
            self.logger.info(f"Loading knowledge base from {self.knowledge_base_file}")
