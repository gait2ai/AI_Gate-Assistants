    ...
    def create_interface(self) -> gr.Blocks:
        ...
        with gr.Blocks(title="AI Gate Knowledge Base Builder", css=self.CUSTOM_CSS) as interface:
            ...
            for btn, process_type in buttons:
                # Create a wrapper function that properly handles the generator
                def create_process_handler(pt):
                    def process_handler():
                        # Get the generator
                        generator = self.manager.run_process(pt, status_box, console_box, *button_components)
                        # Return the generator itself for Gradio to handle
                        return generator
                    return process_handler
                
                btn.click(
                    fn=create_process_handler(process_type),
                    inputs=[],
