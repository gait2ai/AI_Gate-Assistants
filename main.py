...
from modules.website_researcher import KnowledgeBaseResearcher
...
async def initialize_remaining_components() -> bool:
    ...
    try:
        ...
        logger.info("Initializing knowledge base researcher...")
        knowledge_base_file = DATA_DIR / "knowledge_base.json"
        research_config = config.get('website_research', {})
        if not knowledge_base_file.exists():
            logger.error(f"CRITICAL: Knowledge base file not found at {knowledge_base_file}")
            logger.error("Please run the ingestion scripts (website_scraper, document_processor, merge_knowledge) to generate it.")
            raise FileNotFoundError(f"Knowledge base file not found: {knowledge_base_file}")
            
        app_components['knowledge_base_researcher'] = KnowledgeBaseResearcher(
...
async def process_chat_message(user_message: UserMessage, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        research_results = await app_components['knowledge_base_researcher'].search_content(
        ...
        # DEBUG: Log the complete system prompt for debugging
        logger.debug(f"Complete system prompt sent to AI model{session_info}:\n"
                    f"{'='*60}\n"
                    f"{system_prompt}\n"
                    f"{'='*60}")
